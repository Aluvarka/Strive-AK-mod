...
func hunt(person):#agility, strength, endurance, courage
	...
	var dict = {text = text, food = food, supplies = supplies}
	return _brp_job_adjustments(person, dict)
...
func nurse(person):
	...
	var count = 0
	var heal = 0  #Tracks actual HP restored, rather than potential restored
	if globals.player.health < globals.player.stats.health_max:
		heal -= globals.player.health
	...
	var dict = {text = text, heal_oth = heal}
	return _brp_job_adjustments(person, dict)
...
func ffprostitution(person):
	...
	if person.race.find('Bunny') >= 0 || person.spec in ['geisha','nympho']:
		...
		person.energy -= round(rand_range(25,35))
...
func fucktoy(person):
	...
	if person.mods.has("augmenttongue"):
		gold = gold * 1.15
	gold = round(gold)
	person.energy -= round(rand_range(35,50))
	if person.wit >= 25:
		person.loyal -= 8
	if person.conf >= 25:
		person.stress += 15
...
func assistwimborn(person):
	...
	gold = (rand_range(1,5) + person.smaf*15 + person.wit/4 + min(globals.state.reputation.wimborn/1.5,50))/(person.stats.energy_cur/75)
...
func whorewimborn(person):
	...
	if person.race.find('Bunny') >= 0 || person.spec in ['geisha','nympho']:
		person.stress += 12 - min(counter*4, 10)
		person.energy -= round(rand_range(20,30))
	else:
		person.stress += 25 - min(counter*5, 20)
		person.energy -= round(rand_range(35,45))
	if person.spec == 'geisha':
...
func escortwimborn(person):
	...
	if rand_range(1,10) > 7:
		globals.impregnation(person)
	gold = (rand_range(15,35) + person.conf/3 + person.beauty/3 + max(5, person.charm/1.8 + min(globals.state.reputation.wimborn,60)))/(person.stats.energy_cur/80)
	if person.traits.has('Pretty voice') == true:
		gold = gold*1.2
	elif person.traits.has('Foul Mouth') == true:
		gold = gold*0.7
...
func fucktoywimborn(person):
	...
	if person.race.find('Bunny') >= 0 || person.spec == 'nympho':
		...
		person.energy_cur -= round(rand_range(25,35))
	else:
		person.stress += 50 - min(counter*7, 35)
		person.energy -= round(rand_range(40,50))
...
func maid(person):
	...
	var actual_clean = 0 #Used to track actual amount cleaned, rather than potential amount
	var clean = brp_maid_clean_amt(person)
	person.xp += temp/4
	actual_clean -= globals.state.condition
	globals.state.condition = temp
	actual_clean += globals.state.condition
	text = "$name spent the day cleaning around the mansion. \n"

	var dict = {text = text, clean = actual_clean}
	return _brp_job_adjustments(person, dict)


##BRP - The following are a group of helper functions for Jobs
##Calculate amount of cleaning "person" will do in a days
##Returns said value
func brp_maid_clean_amt(person):
	return 5.5 + (person.sagi+person.send)*6

##Calculates the adjusted value for healings based on personal traits or other universal modifiers
##Person argument is the person to be healed
##Returns the new value (does not apply healing)
func brp_heal_calc(person, heal_pct):
	if person.traits.has("Infirm"):
		heal_pct = heal_pct/3

	return heal_pct * person.stats.health_max

##Applies healing to argument "person"
##Function does modify healing amount for universal modifiers prior to applying
func _brp_heal_slave(person, heal_pct):
	person.health += brp_heal_calc(person, heal_pct)

##Universal funtion to apply standard modifiers to jobs and cause secondary features to trigger
##Arguments are the person who is doing a job and the job's dictionary output
##of the work performed (prior to values being applied)
func _brp_job_adjustments(person, dict):
	var traits = person.traits
	var jobtags = jobdict[person.work].tags

	if traits.has("Clumsy") && jobtags.has("physical"):
		if dict.has("gold"):
			dict.gold *= 0.7
		if dict.has("food"):
			dict.food *= 0.7

	if traits.has("Hard Worker") && !jobtags.has("sex"):
		if dict.has("gold"):
			dict.gold *= 1.15

	if person.spec == 'housekeeper' && jobtags.has("mansion"):
		var clean = brp_maid_clean_amt(person)/2
		var actual_clean = 0 #Used to track actual amount cleaned, rather than potential amount
		actual_clean -= globals.state.condition
		globals.state.condition = clean
		actual_clean += globals.state.condition
		if !dict.has("clean"):
			dict.clean = actual_clean
		else:
			dict.clean += actual_clean
		dict.text += person.dictionary("$name has managed to clean the mansion a bit while being around.\n")

	if traits.has("Scoundrel") || traits.has("Lucky") && rand_range(0,100) >= 50 || traits.has("Kleptomania") && rand_range(0,100) >= 50:
		if dict.has("gold"):
			dict.gold += 15
		else:
			dict.gold = 15
		dict.text += "[color=green]$name has brought some additional gold by the end of day.[/color]\n"

	for city in globals.state.reputation:
		if jobtags.has(city):
			if globals.state.reputation[city] < -10 && randf() < 0.33:
				person.obed -= max(abs(globals.state.reputation[city])*2 - person.loyal/6,0)
				person.loyal -= rand_range(1,3)
				dict.text += "[color=#ff4949]$name has been influenced by local townfolk, which is hostile towards you.[/color]\n"
			elif globals.state.reputation[city] > 10 && randf() < 0.2:
				person.obed += abs(globals.state.reputation[city])
				person.loyal += rand_range(1,3)
				dict.text += "[color=green]$name has been influenced by local townfolk, which is loyal towards you.[/color]\n"
