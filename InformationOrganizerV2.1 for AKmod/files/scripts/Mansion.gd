...
var BRPManInfo = load("res://files/scripts/BRP/ManInfo.gd").new()
...
func _ready():
	...
	###BRP - 2019/07/29
	###Changing the mansioninfo panel's url matching to use slave IDs
	$MainScreen/mansion/mansioninfo.connect("meta_hover_started",self,'brp_smart_hover')
	$MainScreen/mansion/mansioninfo.connect("meta_clicked",self, 'brp_smart_clicked')
	$MainScreen/mansion/mansioninfo.connect("meta_hover_ended",self, 'brp_smart_hover_end')
	###BRP - End mod
...
###BRP - 2019/07/29
###Replicating the previous hover and click functions, but
###Basing slave identity on slave ID
###Also adding a link to combat team dialog
func brp_smart_hover(meta):
	if meta == 'combat_launch':
		globals.showtooltip("Combat Group")
	elif meta != 'race':
		var tempslave = globals.state.findslave(int(meta))
		globals.slavetooltip(tempslave)

func brp_smart_clicked(meta):
	globals.slavetooltiphide()
	get_node("FinishDayPanel").hide()
	_on_mansion_pressed()
	yield(self, 'animfinished')
	if meta == 'combat_launch':
		globals.main._on_combatgroup_pressed()
	elif meta != 'race':
		var tempslave = globals.state.findslave(int(meta))
		if !tempslave.work in ['cow', 'hen']:
			globals.openslave(tempslave)
		else:
			_on_farm_pressed()
			farminspect(tempslave)

func brp_smart_hover_end(meta = null):
	globals.hidetooltip()
###BRP - End mod

func slavehover(meta):
	var tempslave
	if meta.find('person') >= 0:
...
func _on_end_pressed():
	...
	#BRP - Extra Variables for Info reporting
	var start_supplies = globals.itemdict.supply.amount
	var start_dirt = globals.state.condition
	var brp_job_reports = JobReport.new()
	...
	for i in range(globals.slaves.size()):
		if globals.slaves[i].away.duration == 0:
			...
			elif globals.slaves[i].work == 'nurse':
				doc = globals.slaves[i]
...
#							else:
#								globals.addrelations(person, i, (rand_range(10,20)))
					#BRP - Original code was hard-coded here for Rest job rather than using Job functions
					#This is still a bit of a hack fix, but required until such time as "work-relations" code is cleaned
					if person.work != jobRestore:
						person.work = jobRestore
						workdict = globals.jobs.call("rest", person)
						brp_job_reports.Builder(person, "rest", workdict)
					else:
						workdict = globals.jobs.call(person.work, person)
						brp_job_reports.Builder(person, person.work, workdict)
						if workdict.has('dead') && workdict.dead == true:
							deads_array.append({number = count, reason = workdict.text})
							continue
					#BRP - Code relocated into Jobs&Specs
				...
				#BRP - function was relocated into jobs, but these 3 positions currently don't exist there
				# if person.spec == 'housekeeper' && person.work in ['rest','cooking','library','nurse','maid','headgirl','farmmanager','labassist','jailer']:
				if person.spec == 'housekeeper' && person.work in ['headgirl','farmmanager','jailer']:
					globals.state.condition += globals.jobs.brp_maid_clean_amt(person)/2.0
			...
			######## Counting food
			if person.traits.has("Baker") && person.level < globals.itemdict.supply.amount && person.sleep != 'jail' && person.sleep != 'farm' || person.traits.has("Kleptomania") && rand_range(0,100) >= 50:
				globals.resources.food += round(person.level*(rand_range(5,15)/rand_range(2,5)))
				...
				if i.has('duration') && i.code != 'captured':
					...
					if i.duration <= 0:
						if i.code == 'captured':
							...
							#BRP - Rather hacky bit to pass this info on to the report
							brp_job_reports.non_rebels.append(person)
						person.add_effect(i, true)
				if i.has("ondayend"):
			...
			if person.traits.has("Uncivilized"):
				for i in globals.slaves:
					if i.spec == 'tamer' && i.away.duration == 0 && i.obed > 60 && \
							( i.work == person.work
								|| i.work in ['rest','nurse','headgirl']
								|| (i.work == 'jailer' && person.sleep == 'jail')
								|| (i.work == 'farmmanager' && person.work in ['cow','hen'])
							):
						person.obed += 30
						person.loyal += 5
						if randf() < 0.1:
			...
			#BRP - Code relocated into Jobs&Specs
			#Causes a small change in functionality
			#Scoundrel trait will no longer get benefit when working a non-"corejobs"
			...
			if person.traits.has("Scoundrel") || person.traits.has("Lucky") && rand_range(0,100) >= 50 || person.traits.has("Kleptomania") && rand_range(0,100) >= 50:
				globals.resources.gold += 15
				text1.set_bbcode(text1.get_bbcode() + person.dictionary('[color=green]$name has brought some additional gold by the end of day.[/color]\n'))
			...
			if person.traits.find("Peaceful mind") > 0:
				if person.stress >= 99:
					person.stress = 99
...
#				text0.set_bbcode(text0.get_bbcode() + person.dictionary("[color=red]$name try to end $his life![/color] \n"))
				...
				person.energy -= rand_range(25,50)
				if randf() <= 0.05:
			...
			if person.stress >= 33 && person.stress <= 65 && randf() <= 0.3:
				if randf() >= 0.5:
			...
			if person.stress >= 66 && person.stress <= 98 && randf() <= 0.3:
				if randf() >= 0.5:
			...
			if person.lust < 25 && person.traits.has('Sex-crazed') && person.sleep != 'your':
				if randf() > 0.9:
			...
			if person.traits.has('Broken limb') || person.traits.has('Cracked rib') || person.traits.has('Bruised') || person.traits.has('Heavy injured') || person.traits.has('Sickness'):
				var toheal = variables.chancetoheal
				if person.traits.has('Regeneration'):
					toheal += 10
				if person.sleep == 'personal' || person.sleep == 'your':
					toheal += 5
				if doc != null:
					toheal += 10
				if person.traits.has("Infirm") || person.traits.has("Glass Bones"):
					toheal /= 2
				if rand_range(0,100) <= toheal:
					if person.traits.has('Broken limb'):
						person.trait_remove('Broken limb')
						text0.bbcode_text += person.dictionary("[color=green]$name Broken limb complete heal.[/color]\n")
				if rand_range(0,100) <= toheal:
					if person.traits.has('Cracked rib'):
						person.trait_remove('Cracked rib')
						text0.bbcode_text += person.dictionary("[color=green]$name arm complete heal.[/color]\n")
				if rand_range(0,100) <= toheal+3:
					if person.traits.has('Bruised'):
						person.trait_remove('Bruised')
						text0.bbcode_text += person.dictionary("[color=green]$name bruises complete heal.[/color]\n")
				if rand_range(0,100) <= toheal-3:
					if person.traits.has('Heavy injured'):
						person.trait_remove('Heavy injured')
						text0.bbcode_text += person.dictionary("[color=green]$name healthy again.[/color]\n")
				if rand_range(0,100) <= toheal+5:
					if person.traits.has('Sickness'):
			...
			if person.lactation == true && !person.traits.has("Love childs"):
				if rand_range(0,10) >= 8 && person.sleep != 'farm' && person.preg.duration <= 5:
					person.lactation = false
					text0.set_bbcode(text0.get_bbcode()+person.dictionary('[color=yellow]$name stop lactating.[/color]\n'))
				person.lust += rand_range(1,10)
			if person.preg.duration > 0:
				person.preg.duration += 1
				if rand_range(0,100) <= 51 && !person.traits.has('Sickness') && globals.player != person:
					person.add_trait('Sickness')
			...
			if person.away.at == 'lab' && person.health < 5:
				...
				if person.away.duration == 0:
					...
					person.away.at = ''
					brp_job_reports.Builder(person, person.work, {"returned" : true})
				for i in person.effects.values():
					if i.has('duration') && i.code != 'captured':
						if person.race != 'Dark Elf' || (!i.code in ['bandaged','sedated'] && randf() > 0.5):
							i.duration -= 1
		...
		for person in globals.slaves:
			if person.sleep == 'farm' && person.away.duration == 0:
				...
				if person.farmoutcome == false:
					brp_job_reports.Builder(person, person.work, {"food": production})
					...
					brp_job_reports.Builder(person, person.work, {"gold": round(production/2)})
	...
	if globals.state.mansionupgrades.foodpreservation == 0 && globals.resources.food >= globals.resources.foodcaparray[globals.state.mansionupgrades.foodcapacity]*0.01:
		globals.resources.food -= globals.resources.food*float(rand_range(0.00,0.08))
	...
	#BRP - Generate the new summary report tab
	_brp_add_report(brp_job_reports, start_gold, start_food, start_supplies, start_dirt)
...
##BRP - Call this function after daily jobs/activities have been processed to
##generate a summary report in a newly added 4th tab.
func _brp_add_report(brp_job_reports, start_gold, start_food, start_supplies, start_dirt):
	_brp_init_report_pane()
	_brp_report_container.generate_report(brp_job_reports, start_gold, start_food, start_supplies, start_dirt)

##BRP - The following method is a run-once function to generate an additional reports
##  tab for end of day info.  The resulting var provides the hook to fill the new()
##  tab with current-day report information.
var _brp_report_container
func _brp_init_report_pane():
	if _brp_report_container:
		return

	_brp_report_container = load("res://files/scripts/BRP/Report.gd").new()
	var tabs = get_node("FinishDayPanel/FinishDayScreen")
	tabs.add_child(_brp_report_container.root)
	tabs.set_tab_title(3, "Summary")
	tabs.move_child(_brp_report_container.root, 0)
...
#
	...
	if globals.state.dailyeventcountdown <= 0 && !$scene.is_visible_in_tree() && !$dialogue.is_visible_in_tree():
		...
		while moarevents > 0:
			...
			moarevents -= 1
		if event != null:
			if variables.dailyeventstime != 5:
				globals.state.dailyeventcountdown = rand_range(timebeetwen,timebeetwen*2)
...
func _on_mansion_pressed():
	...
	# build_mansion_info()
	# man_info_node.set('custom_fonts/normal_font', load(BRPManInfo.defaultFont))
	textnode.show()
	textnode.set_bbcode(BRPManInfo._format_mansion_status(MAGE_GUILD_TITLE))
...
var statedict = {
	health = {high = 'Healthy', med = 'Injured', low = 'Wounded'},
	energy = {high = 'Lively', med = 'Tired', low = 'Wasted'},
	stress = {high = "Content", med = 'Stressed', low = "On verge"}
}
...
func build_mansion_info():
	...
	var unassigned = "[color=yellow]Unassigned[/color]"
	...
	if sleepers.communal > globals.state.mansionupgrades.mansioncommunal:
		...
		text += colordict.high
	text += str(sleepers.communal) + "/" + str(globals.state.mansionupgrades.mansioncommunal) +  '[/color] beds in the communal room\n'
	
	text += 'You have ' + globals.fastif(sleepers.personal >= globals.state.mansionupgrades.mansionpersonal, '[color=yellow]', '[color=green]') + str(sleepers.personal) + "/" + str(globals.state.mansionupgrades.mansionpersonal) + '[/color] ' + globals.fastif(globals.state.mansionupgrades.mansionpersonal > 1, 'personal rooms', 'personal room')+ ' available for living\nYour bed can fit ' +globals.fastif(sleepers['your_bed'] >= globals.state.mansionupgrades.mansionbed, '[color=yellow]', '[color=green]') + str(sleepers.your_bed) + "/" + str(globals.state.mansionupgrades.mansionbed) + '[/color] ' +  globals.fastif(globals.state.mansionupgrades.mansionpersonal > 1, 'persons', 'person')+' besides you.'
	
	text += '\n\nYour jail has ' + globals.fastif(sleepers.jail >= globals.state.mansionupgrades.jailcapacity, '[color=yellow]', '[color=green]') +str(sleepers.jail) + "/" + str(globals.state.mansionupgrades.jailcapacity) +'[/color] cells. \n'
	if globals.state.farm >= 3:
		text += "Your farm holds " + globals.fastif(sleepers.farm >= variables.resident_farm_limit[globals.state.mansionupgrades.farmcapacity], '[color=yellow]', '[color=green]') +str(sleepers.farm) + "/" + str(variables.resident_farm_limit[globals.state.mansionupgrades.farmcapacity]) +'[/color] booths for cattle. \n'
	...
	if globals.state.playergroup.size() <= 0:
		...
		for i in globals.state.playergroup.duplicate():
			...
			if person != null:
				...
				text += person.dictionary('[url=person' + str(counter) + '][color=aqua]$name[/color][/url]: ') 
				var value
				if float(person.stats.health_cur)/person.stats.health_max > 0.75: 
					value = 'high'
				elif float(person.stats.health_cur)/person.stats.health_max > 0.4:
					value = 'med'
				else:
					value = 'low'
				...
				if float(person.stats.energy_cur)/person.stats.energy_max > 0.5: 
					...
					value = 'low'
				text += colordict[value] + statedict.energy[value] + "[/color], "
				...
				if person.stress < 50: 
					...
					value = 'low'
				text += colordict[value] + statedict.stress[value] + "[/color]\n"
...
func childbirth(person):
	...
	if !person.traits.has("Love childs") && globals.player != person && rand_range(0,10) > 7:
		person.add_trait("Love childs")
...
func _on_selfbutton_pressed():
	...
	## BRP - Moved to file-level variable for multi-access
	# var dict = {
	# 0: "You do not belong in an Order.",
	# 1: "Neophyte",
	# 2: "Apprentice",
	...
	text += "\nYour mage order rank: " + MAGE_GUILD_TITLE[int(globals.state.rank)]
