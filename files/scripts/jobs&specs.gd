...
var jobdict = {
	...
	library = {
		...
		reqs = "globals.currentslave.traits.has('Illiterate') == false",
		unlockreqs = 'true',
		maxnumber = 0,
		order = 6,
		location = 'mansion',
	...
	assistwimborn = {
		...
		reqs = "globals.originsarray.find(globals.currentslave.origins) >= 2 && globals.currentslave.traits.has('Mute') == false && globals.currentslave.traits.has('Illiterate') == false",
...
var specs = {
	...
	merchant = {
		...
		reqs = "person.wit >= 50 && person.charm >= 50 && globals.currentslave.traits.has('Illiterate') == false"
	},
	tamer = {
		name = "Tamer",
		code = 'tamer',
