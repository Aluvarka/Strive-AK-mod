...
func genitals():
	...
	if globals.state.descriptsettings.genitals == true || showmode != 'default':
		text = "\n" + text + getdescription("titssize") + getlactation() + gettitsextra() + getdescription("asssize") + lowergenitals()
...
func getlactation():
	...
	if person.lactation == true:
		if person.titssize == 'big' || person.titssize == 'huge':
			text += 'Stream of [color=yellow]nutrious milk[/color] leaking down. '
		else:
			text += 'Drops of [color=yellow]milk[/color] ocassionaly leaked down. '
	else:
		text = ''
	return text
...
wings = {
	...
	leather_black = 'Hidden on $his back is a pair of [color=aqua]bat-like, leather wings[/color]. ',
	leather_red = 'Hidden on $his back is a pair of [color=aqua]dragon-like, leather wings[/color]. ',
},
tail = {
	none = '',
