...
class combatant:
	...
	func createfromslave(person, data = null):
		...
		if person.spec == 'assassin':
			speed += 5
			attack += 5
		if person.traits.find("Claws and Fangs") >= 0:
			attack += 2
		if person.traits.find("Heavy Hooves") >= 0:
			attack += 2
			speed -= 2
		if person.traits.find("Dodger") >= 0:
			speed += 3
		if person.traits.find("Brute") >= 0:
			speed -= 3
			attack += 3
		if person.traits.find("Bestial Instinct") >= 0:
			speed += 2
		if person.traits.find("Best Flyer Ever") >= 0:
			speed += 5
		if person.traits.find("Horns&Hooves") >= 0:
			attack += 4
		...
		if person.traits.find("Lucky") >= 0:
			...
			if rand_range(0,100) <= 25:
				if rand_range(0,100) <= 50:
					...
					speed += 10
			elif rand_range(25,100) <= 50:
				if rand_range(0,100) <= 50:
					attack += 6
				else:
					attack += 12
			elif rand_range(50,100) <= 75:
				if rand_range(0,100) <= 50:
					armor += 6
				else:
					armor += 9
			...
			if i != null:
				...
				for k in tempitem.effects:
					...
					if k.effect == 'combatstress':
						stress += passives.combatstress.effectvalue
	...
	func defeat():
		...
		if group == 'player':
			...
			if person == globals.player:
				...
				elif _slave.traits.has("Nine Lives"):
					_slave.trait_remove("Nine Lives")
					_slave.health += 9
...
func physdamage(caster, target, skill):
	...
	if caster.passives.has('exhaust'):
		power = power * 0.66
	if target.passives.has('ranged') && !target.passives.has('protecteffect'):
...
func calculatehit(caster,target,skill):
	...
	if caster.passives.has('ranged'):
		hitchance = hitchance*1.25
	if caster.stress != null && caster.stress >= 66:
