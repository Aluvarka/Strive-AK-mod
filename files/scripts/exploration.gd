...
func nothing():
	var array = []
	if randf() >= 0.50:
		mansion.maintext = "Summer sun, light breeze, singing birds, and no one met on your way. "
	else:
		mansion.maintext = "Except for the dust and you, the area is empty. "
...
func enemydefeated():
	...
	for unit in enemygroup.units:
		...
		for i in unit.rewardpool:
			...
			if globals.state.spec == "Hunter":
				bonus += 0.2
			chance = chance*bonus
			foodearned += round((unit.rewardfood*bonus)*randf())
	...
	if foodearned > 0:
		globals.resources.food += foodearned
		text += '\nYou have foraged [color=yellow]' + str(round(foodearned)) +'[/color] pieces of meat from fallen creatures. \n'
