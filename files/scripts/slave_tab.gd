...
func _ready():
	...
	randPortrait.text = "Random Portrait"
	randPortrait.disconnect("pressed", self, '_on_portrait_pressed')
	randPortrait.connect("pressed", self, 'regenerateportrait')
	randPortrait.margin_top -= 48
	randPortrait.margin_bottom -= 48
	randPortrait.margin_left -= 12
	$stats/customization.add_child(randPortrait)
	# Random Portrait button end
	# Select naked image button start
	var naked = $stats/customization/portrait.duplicate()
	naked.text = "Naked Image"
	naked.disconnect("pressed", self, '_on_portrait_pressed')
	naked.connect("pressed", self, 'naked')
	naked.margin_top -= 96
	naked.margin_bottom -= 96
	naked.margin_left += 0
	# Random Portrait button end
