...
func invigorateeffect():
	...
	if person.effects.has('invigorated') == false:
		globals.resources.mana -= spellcost(spell)
		person.energy += person.stats.energy_base
		person.stress += max(rand_range(25,35)-globals.player.smaf*4, 10)
		globals.player.energy += 100
		text = person.dictionary("You cast Invigorate on $name. Your and $his energy is fully restored. $His stress has increased. ")
		person.add_effect(globals.effectdict.invigorated)
	else:
		globals.resources.mana -= spellcost(spell)
		person.energy += person.stats.energy_base/2
		person.stress += max(rand_range(25,35)-globals.player.smaf*4, 10)
		globals.player.energy += 50
		person.toxicity += round(rand_range(3,5))
		text = person.dictionary("Today you are already cast Invigorate on $name. Your and $his energy is only partly restored. $His stress has increased. ")
	return text
