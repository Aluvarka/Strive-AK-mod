...
####Resources
var textnode = globals.questtext #Old quest system text
...
func tishaappearance():
	...
	if emily == null:
		return
	if emily.away.duration > 0:
		globals.state.upcomingevents.append({code = 'tishaappearance', duration = emily.away.duration + 2})
		return
...
func tishadisappear(stage = 0):
	...
	if emily == null:
		return
	if emily.away.duration > 0:
		globals.state.upcomingevents.append({code = 'tishadisappear', duration = emily.away.duration + 2})
		return
	if stage == 0:
...
var bossloot = [["armorplate+",1],['weaponsword+',1],['weaponclaymore+',1],['weaponcursedsword',1],['weaponhammer+',1],['weaponkatana+',1],['weaponshortsword+',1],['armorcarapace',1],['armorredcloak',1], ['accessoryneck+',1],['armorrogue',1],['armortentacle',1],['weaponelvensword+', 1],['crossbow+',1],['vhelmet+', 1] ]
