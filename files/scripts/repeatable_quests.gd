...
static func commonrace():
	var text
	text = ['Human','Elf','Tribal Elf','Halfkin Cat','Halfkin Wolf','Halfkin Fox','Halfkin Bunny','Taurus','Demon','Goblin','Ork']
...
static func rarerace():
	var text
	text = ['Dragonkin','Harpy','Arachna','Lamia','Nereid','Scylla','Demon','Seraph','Dark Elf','Slime','Fairy','Centaur','Gnome']
	if globals.rules.furry == true:
...
func questarray():
	var questsarray = {
		...
		quest003 = {
			...
			reward = round(rand_range(85,140))*10,
			location = ['any'],
			difficulty = 'medium',
		},
		quest004 = {
		...
		quest026 = {
			...
			shortdescription = 'Exquisite collector looking for a rare species.',
			description = "A particularly elegant letterhead bears the nuanced description of a slave desired by a famously eccentric collector. A handsome reward is offered for the delivery of a $sex $race.",
			reqs = [['obed','gte', 100],['beauty','gte', 75]],
			reqstemp = [['age','eq',randage()],['race','eq',rarerace()],['sex', 'eq', randsex()]],
			reqsfunc = ['nobadtraits'],
			time = round(rand_range(6,12)),
			reward = round(rand_range(136,185))*10,
			location = ['any'],
			difficulty = 'medium',
		},
		quest027 = {
...
func generatequest(town, difficulty = 'easy'):
	...
	var quest = globals.randomfromarray(questarray2)
	if town == 'wimborn':
		...
		globals.state.repeatables.gornslaveguild.sort_custom(self, 'sort_difficulty')
	elif town == 'frostford':
		globals.state.repeatables.frostfordslaveguild.append(quest)
		globals.state.repeatables.frostfordslaveguild.sort_custom(self, 'sort_difficulty')
