...
func loadsettings():
	...
	var baseFolders = {portraits = appDataDir +'portraits/', fullbody = appDataDir + 'bodies/', fullnaked = appDataDir + 'exposed/', mods = appDataDir + 'mods/'}
...
class progress:
	...
	func calculateweight():
		...
		for i in globals.state.playergroup:
			...
			if _slave.traits.has('Strong back') == true:
				maxweight += 15
...
func addrelations(person, person2, value):
	if person == player || person2 == player || person == person2 || person2 == null:
		return
...
func itemdescription(item, short = false):
	...
	if item.has('owner'):
		...
		if item.enchant == 'basic':
			...
			name = '[color=#9900cc]' + name + '[/color]'
		elif item.enchant == 'cursed':
			name = '[color=red]'+ name + '[/color]'
		elif item.enchant == 'bagzbunny':
